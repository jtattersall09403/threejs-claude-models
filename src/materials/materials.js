...
// Exponent 4 makes the triplanar blend nearly a hard switch: where the normal crosses
  // a 45 degree diagonal the dominant plane swaps, and on a rippled surface it crosses
  // back and forth repeatedly, so the three planes' differing samples show up as
  // alternating hard-edged strips. On cloth, amplified by a 2.5x height-to-value swing,
  // that reads as torn or peeling geometry at grazing angles.
  vec3 w = pow(abs(n), vec3(2.2));
  w /= (w.x + w.y + w.z + 1e-5);
...
// The head's scales have to be BIG enough to read. At 25 cycles/m the muzzle's plates
  ...
  // debugMasks(1), red channel: this term evaluated to 1 across the ENTIRE cranium and
  // upper face — everything above J.y 1.672 and behind J.z 0.122 — so plate * 0.72,
...
// A DARK MASK around the eye. In every reference the eye sits in a near-black socket
  ...
  // Centred ON the brow ridge as it is actually built (y ~1.735-1.75, z ~0.07-0.12).
  ...
  // A LIT UPPER LIP above the dark line. debugMasks(6) showed the lip mask present,
  // correctly placed and running the full muzzle — the line was simply dark-on-dark and
  // had nothing to read against. A mouth is legible because of the highlight on the lip
  // over the groove, not because of the groove.
  float lipEdge = ss(0.0082, 0.0040, abs(H.y - (lipY + 0.0068)))
                * ss(0.200, 0.186, H.z) * ss(-0.034, -0.006, H.z);
  col = mix(col, col * 1.75 + vec3(0.0034, 0.0036, 0.0024), lipEdge * 0.58);
  ...
  // The head's damping used to be 0.92..1.03 — a five per cent swing — on the theory
  // that the bright reticulation below would supply the contrast instead. Once that net
  // was narrowed back to crevice lines, the head was left with essentially NO scale
  // value contrast at all, and the muzzle rendered as a large smooth pale expanse next
  // to a reference muzzle that is dark and densely patterned. Damped, but not erased.
  col *= mix(mix(0.52, 1.04, ss(0.02, 0.55, h)),
             mix(0.68, 1.06, ss(0.02, 0.55, h)), headMask);
  ...
  col = mix(col, vec3(0.0640, 0.0715, 0.0428), mortar * 0.95);
...
export function createMaterials() {
  ...
  return {
    ...
    // Darker. Against the reference the coat is a near-charcoal brown; ours rendered as a
    // light khaki, which changes the whole figure's read from a labourer in a worn dark
    // coat to one in pale canvas.
    tunic: clothMat('tunic', [0.0096, 0.0080, 0.0068], 0.95, 9.0, cloth),
