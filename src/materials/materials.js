...
const HORN_FRAG = /* glsl */`
  ...
  // The CROWN CREST and BROW SPINES are dark oxblood in the references — see
  // face-front-and-bust-proportions.jpg, where they read as a red-brown fan against
  // the skull. Only the horns and the jaw spikes are pale bone. Keyed on height
  // rather than region so the jaw spikes (low) keep their cream.
  float crest = ss(1.680, 1.715, vRest.y) * (1.0 - step(0.5, vRegion));
  col = mix(col, vec3(0.0362, 0.0128, 0.0106) * (0.72 + 0.55 * grime), crest * 0.90);
  gRoughOut = mix(gRoughOut, clamp(gRoughOut + 0.14, 0.3, 0.95), crest);
