...
export function clothingFields(body) {
  ...
  // ---- undershirt: thin, only visible at the collar and the V of the tunic ----
  {
    ...
    f.add(capsule([0, 1.384, -0.008], [0, 1.426, 0.006], 0.108, 0.092, { k: 0.024 }));
    f.add(ellipsoid([0, 1.390, 0.038], [0.090, 0.040, 0.064], { k: 0.020 }));  // knotted front
    ...
    for (const [y, z, r] of [[1.372, 0.030, 0.098], [1.392, 0.026, 0.100], [1.412, 0.020, 0.096]]) {
    ...
    f.add(capsule([0, 1.406, 0.010], [0, 1.410, -0.006], 0.098, 0.096, { k: 0.012 }));
    f.add(ellipsoid([0.030, 1.412, 0.066], [0.036, 0.026, 0.030], { k: 0.018 }));
    ...
    f.add(ellipsoid([0, 1.386, -0.074], [0.092, 0.044, 0.044], { k: 0.030 }));
    f.add(ellipsoid([0, 1.418, -0.086], [0.070, 0.032, 0.032], { k: 0.028 }));
    f.sub(capsule([0, 1.412, -0.016], [0, 1.70, 0.014], 0.066, 0.076, { k: 0.018 })); // neck hole
...
// THE SKIRT MUST CLEAR THE LEGS. When the stance was widened the thigh's outer surface
    ...
    f.sub(capsule([0, 1.400, 0.066], [0, 1.60, 0.010], 0.050, 0.078, { k: 0.028 }));
    // rolled collar band around the opening, so the edge reads as a hem
    f.add(capsule([0, 1.400, 0.040], [0, 1.430, 0.030], 0.074, 0.072, { k: 0.018 }));
    f.sub(capsule([0, 1.392, 0.068], [0, 1.62, 0.008], 0.046, 0.072, { k: 0.020 }));
