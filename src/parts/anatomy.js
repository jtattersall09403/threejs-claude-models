...
export const LIP = { y0: 1.6292, z0: 0.150, slope: 0.290 };
...
export function buildHeadField() {
  ...
  for (const s of [1, -1]) {
    ...
    f.add(ellipsoid([s * 0.0512, 1.7455, 0.066], [0.034, 0.0180, 0.046], { k: 0.026 }));  // brow shelf, outer
    f.add(ellipsoid([s * 0.0280, 1.7365, 0.080], [0.026, 0.0162, 0.038], { k: 0.024 })); // ...dipping inboard
    f.add(ellipsoid([s * 0.0630, 1.6905, 0.022], [0.018, 0.054, 0.055], { k: 0.035 }));  // temple
  }

  f.add(capsule([-0.030, 1.7395, 0.078], [0.030, 1.7395, 0.078], 0.0136, 0.0136,
...
// The face BELOW the eyes has to stay broad. Narrowing the jaw to fix a slab-like
  ...
  f.add(ellipsoid([0, 1.6388, 0.1700], [0.0154, 0.0134, 0.0120], { k: 0.016 })); // nose pad
  ...
  f.sub(creaseSlot((z) => LIP.y0 + (LIP.z0 - z) * LIP.slope, 0.0042, [-0.005, 0.160],
    (z) => 0.046 - 0.148 * Math.max(0, z - 0.030),
    { k: 0.0045, yMin: 1.56, yMax: 1.68, xBound: 0.07 }));
  // nostrils — at the old size they were below the bake resolution and invisible
  for (const s of [1, -1]) {
    f.sub(ellipsoid([s * 0.0100, 1.6620, 0.1560], [0.0056, 0.0070, 0.0106], { k: 0.0035 }));
  }
  // ear depression
  for (const s of [1, -1]) {
