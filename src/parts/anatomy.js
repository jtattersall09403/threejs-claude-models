...
export function buildBodyField() {
  ...
  for (const s of [1, -1]) {
    ...
    f.add(capsule([s * 0.098, 0.53, 0.012], [s * 0.1, 0.105, -0.012], 0.058, 0.033, { k: 0.045 }));
        f.add(ellipsoid([s * 0.098, 0.522, 0.020], [0.052, 0.038, 0.050], { k: 0.035 }));   // knee
    f.add(ellipsoid([s * 0.1, 0.432, -0.034], [0.049, 0.082, 0.048], { k: 0.05 }));     // calf
...
export function buildHeadField() {
  ...
  f.add(ellipsoid([0, 1.684, -0.010], [0.083, 0.068, 0.100], { k: 0.055 }));
  f.add(roundBox([0, 1.664, -0.056], [0.055, 0.046, 0.030], 0.026, { k: 0.05 }));  // squared occiput
  ...
  f.sub(creaseSlot((z) => LIP.y0 + (LIP.z0 - z) * LIP.slope, 0.0024, [0.028, 0.208], 0.084,
