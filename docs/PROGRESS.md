...
| 8 | jaw/cheek spikes are cones not blades | **DONE — root cause found.** `spike()` swept a PARALLEL-TRANSPORT frame, which picks its starting normal from whichever world axis is least parallel to the tangent, so `opts.flat` set the cross-section's aspect but not which way the flat face POINTED — across a fanned row they came out essentially random. That is why widening them never made them read flat. `spike()` now takes an explicit frame (`opts.faceUp`). Superseded note: **PARTIAL — and there is a real blocker.** Widened and shortened per the critic, but they still do not read flat, and now I know why: `spike()` sweeps along a PARALLEL-TRANSPORT frame, so `opts.flat` controls the cross-section's aspect but NOT which way the flat face points. It comes out effectively random per spike. Making these read as blades needs an explicit frame in `spike()` (an `up` hint, as `buildBelt` does with `frameFn`). |
| 9 | hands read as a rake | **DONE** — stronger quadratic diameter taper, deeper knuckle swellings, and UNEQUAL per-finger curl (equal curl keeps fingers parallel however well they are shaped). The `hands` capture framing was also stale after the arms were re-posed and was shooting the coat; re-aimed. |
